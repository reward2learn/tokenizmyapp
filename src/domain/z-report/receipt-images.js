...
export function imageToDataUrl(img) {
    if (!img?.data)
        return '';
    if (String(img.data).startsWith('data:'))
        return img.data;
...
export function mergeReceiptImages(existing, incoming) {
    ...
    if (!existingSanitized.length)
        return incomingSanitized;
    ...
    for (const img of existingSanitized) {
        if (!incomingSanitized.some((inc) => inc.data === img.data)) {
            throw new Error('Saved receipt images cannot be removed or replaced. You may only add new images.');
        }
    }
