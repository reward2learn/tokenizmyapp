import { isExcelLedgerMonth, lineValueFromPnlLines, manualFieldKeys, num, parsePnlLines, sanitizeManualInputs, } from '@/domain/financial/pnl-calculator';
...
function inputsFromPnlLines(lines) {
    ...
    for (const key of manualFieldKeys()) {
        ...
        if (v != null)
            out[key] = key.endsWith('_count') ? Math.round(v) : v;
    }
    return out;
}
...
export class MonthlyActualsPrefill {
    ...
    async getPriorMonthBaseline(period) {
    ...
    async buildPrefillInputs(period, zAgg, options = {}) {
