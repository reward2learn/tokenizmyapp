...
export async function ensureAppSettingsTable(db) {
    ...
    try {
        await db.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS idx_app_settings_tenant_slug ON app_settings(tenant_slug);');
    }
    catch {
 ...
 */
export async function getAppSettings(db, tenantSlug) {
    ...
    if (existing) {
        ...
        return {
            ...
            tenantMetadata: (ex.tenantMetadata ?? ex.tenant_metadata ?? {}),
    ...
    return {
        ...
        tenantMetadata: (cr.tenantMetadata ?? cr.tenant_metadata ?? {}),
...
export async function updateAppSettings(db, patch, tenantSlug) {
    await ensureAppSettingsTable(db);
    const id = tenantSlug ?? APP_SETTINGS_ID;
    // Build Prisma update data — map camelCase patch keys to Prisma model field names
    const data = {};
    if (patch.webSearchEnabled !== undefined)
        data.webSearchEnabled = patch.webSearchEnabled;
    if (patch.tenantSlug !== undefined)
        data.tenantSlug = patch.tenantSlug;
    if (patch.tenantDisplayName !== undefined)
        data.tenantDisplayName = patch.tenantDisplayName;
    if (patch.tenantTemplate !== undefined)
        data.tenantTemplate = patch.tenantTemplate;
    if (patch.tenantMetadata !== undefined)
        data.tenantMetadata = patch.tenantMetadata;
    if (patch.brandLogoText !== undefined)
        data.brandLogoText = patch.brandLogoText;
    if (patch.brandLogoUrl !== undefined)
        data.brandLogoUrl = patch.brandLogoUrl;
    if (patch.brandPrimaryColor !== undefined)
        data.brandPrimaryColor = patch.brandPrimaryColor;
    if (patch.brandSecondaryColor !== undefined)
        data.brandSecondaryColor = patch.brandSecondaryColor;
