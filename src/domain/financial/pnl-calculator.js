...
export function buildComputedPnl(period, zAgg, manualInputs, ytdBefore) {
    ...
    if (v.gross_profit != null && v.total_overhead != null) {
        ...
        if (revenue)
            v.ebitda_margin = (v.ebitda / revenue) * 100;
    }
    ...
    if (monthNet || priorNet)
        v.net_income_pre_tax_accum = priorNet + monthNet;
    if (monthIncome || priorIncome)
        v.total_income_accum = priorIncome + monthIncome;
    return v;
}
...
export function lineValueFromPnlLines(lines, key) {
    if (!lines)
        return null;
    const row = lines.find((l) => l.key === key);
    return row && row.value != null ? num(row.value) : null;
}
export function parsePnlLines(val) {
    if (!val)
        return [];
