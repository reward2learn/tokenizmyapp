 ...
 */
export async function provisionGoogleOAuth(config) {
    ...
    throw new Error(`[google-cloud] All OAuth provisioning strategies failed for "${slug}".\n` +
        `  To fix:\n` +
 ...
 */
async function tryStrategyServiceAccount(config) {
 ...
 */
async function createProject(accessToken, projectId, displayName) {
 ...
 */
async function enableOAuthApi(accessToken, projectId) {
 ...
 */
async function configureConsentScreen(accessToken, projectId, config) {
 ...
 */
async function createOAuthClient(accessToken, projectId, config) {
 ...
 */
async function listOAuthClients(accessToken, projectId) {
    const url = `https://oauth2.googleapis.com/v2/oauth-clients?projectId=${projectId}`;
    const res = await fetch(url, {
        headers: { Authorization: `Bearer ${accessToken}` },
    });
 ...
 */
function buildClientSecretJson(client, projectId) {
 ...
 */
export async function uploadOAuthLogo(imageBase64, mimeType) {
    const auth = await getServiceAccountToken();
    if (!auth) {
...
// ── Strategy B: gcloud CLI ──────────────────────────────────
function tryStrategyGcloud(config) {
    const { slug, displayName, redirectUris, adminEmail } = config;
    try {
    ...
    try {
        ...
        const account = execSync('gcloud config list account --format="value(core.account)" 2>/dev/null', { stdio: 'pipe', timeout: 5000, encoding: 'utf8' }).toString().trim();
        if (!account) {
        ...
        try {
            execSync(`gcloud projects create ${projectId} --name="${displayName}" --labels=managed_by=tokenizmyapp,tenant=${projectId} --format=json`, { stdio: 'pipe', timeout: 30000 });
            ...
            console.warn('[google-cloud] Project creation (may already exist):', projErr.stderr?.toString('utf8').slice(0, 200));
        }
        // Enable OAuth API
        try {
            execSync(`gcloud services enable oauth2.googleapis.com --project=${projectId}`, { stdio: 'pipe', timeout: 30000 });
        }
        catch {
        ...
        const clientJson = execSync(`gcloud alpha identity oauth-client create "${displayName} (${slug})" ` +
            `--project=${projectId} ` +
            `--display-name="${displayName} (${slug})" ` +
            `--type=web ` +
            `${redirectArg} ` +
            `--support-email="${adminEmail}" ` +
            `--format=json 2>/dev/null`, { stdio: 'pipe', timeout: 30000, encoding: 'utf8' }).toString();
        ...
        const clientSecretJson = buildClientSecretJson({ client_id: parsed.clientId, client_secret: parsed.clientSecret, display_name: displayName, redirect_uris: redirectUris }, projectId);
        return {
        ...
        console.warn('[google-cloud] gcloud strategy failed:', err.message);
        return null;
    }
}
// ── Strategy C: Env fallback ─────────────────────────────────
function envVarFallback(config) {
    const clientId = process.env.GOOGLE_CLIENT_ID;
    const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
    const projectId = process.env.GOOGLE_PROJECT_ID || 'shared-oauth-project';
    if (!clientId || !clientSecret)
        return null;
    const clientSecretJson = buildClientSecretJson({ client_id: clientId, client_secret: clientSecret, display_name: config.displayName, redirect_uris: config.redirectUris }, projectId);
    return {
        clientId,
        clientSecret,
        projectId,
 ...
 */
export function formatGoogleOAuthResult(result) {
    return `
╔══════════════════════════════════════════════════════════════╗
 ...
 */
export async function saveClientSecretJson(result) {
 ...
 */
export async function deleteGoogleOAuthCredentials(clientId, projectId) {
