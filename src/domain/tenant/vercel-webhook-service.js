 ...
 */
async function findTenantByVercelProject(projectIdOrName) {
    const db = createRawClient(); // raw for reliability in webhook context
    try {
        // Try exact project ID match
        const tenant = await db.$queryRawUnsafe(`SELECT id, slug, vercel_project_id FROM tenants 
       WHERE vercel_project_id = $1 OR slug = $1 OR display_name ILIKE $2 
       LIMIT 1`, projectIdOrName, `%${projectIdOrName}%`);
 ...
 */
async function recordWebhookEvent(eventType, payload, status, errorMessage, durationMs) {
    try {
        const db = createBaseClient();
        // After zenstack generate, this will work. For now, use raw insert if model ready.
        await db.$executeRawUnsafe(`INSERT INTO webhook_events (id, event_type, payload, status, error_message, duration_ms, created_at)
       VALUES (cuid(), $1, $2, $3, $4, $5, NOW())`, eventType, JSON.stringify(payload), status, errorMessage || null, durationMs || null);
 ...
 */
async function updateTenantStatus(projectId, status) {
    const db = createRawClient();
    try {
        await db.$executeRawUnsafe(`UPDATE tenants 
       SET status = $1, updated_at = CURRENT_TIMESTAMP 
       WHERE vercel_project_id = $2`, status, projectId);
 ...
 */
export async function handleVercelWebhook(rawBody, headers) {
    const startTime = Date.now();
    // Safe header extraction compatible with both Headers and plain object
    let signature = null;
    let eventTypeHeader = null;
    if (headers instanceof Headers || (typeof headers.get === 'function')) {
        signature = headers.get('x-vercel-signature') || headers.get('X-Vercel-Signature');
        eventTypeHeader = headers.get('x-vercel-event');
    }
    else {
        const h = headers;
        signature = (h['x-vercel-signature'] || h['X-Vercel-Signature'] || '');
        if (Array.isArray(signature))
            signature = signature[0];
        eventTypeHeader = (h['x-vercel-event'] || '');
        if (Array.isArray(eventTypeHeader))
            eventTypeHeader = eventTypeHeader[0];
    }
    let event;
    try {
