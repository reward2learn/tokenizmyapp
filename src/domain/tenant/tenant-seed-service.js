 ...
 */
export async function addTenantColumnsIfMissing(db) {
    const statements = [
    ...
    for (const sql of statements) {
        try {
            ...
            console.warn(`[tenant-seed] Migration warning (non-fatal):`, err.message);
        }
    }
}
/**
...
export async function seedTenantDefaults(input) {
    ...
    try {
        await db.$executeRawUnsafe(`INSERT INTO app_settings (id, web_search_enabled, tenant_slug, tenant_display_name, tenant_template,
       ...
       ON CONFLICT (id) DO UPDATE
         SET tenant_slug = $2, tenant_display_name = $3, tenant_template = $4,
             brand_primary_color = $5, brand_secondary_color = $6, updated_at = NOW();`, input.slug, input.slug, input.displayName, input.template, input.primaryColor, input.secondaryColor);
    ...
    for (const tplPage of template.defaultPages) {
        try {
            ...
            await db.$executeRawUnsafe(`INSERT INTO app_pages (id, slug, title, auth_tier, sort_order, nav_label, show_in_nav, tenant_slug)
         ...
         ON CONFLICT (slug) DO UPDATE
           SET id = COALESCE(app_pages.id, $1), title = $3, auth_tier = CAST($4 AS "AuthTier"), sort_order = $5,
               nav_label = $6, show_in_nav = true, tenant_slug = $7;`, pageId_, tplPage.slug, tplPage.title, tplPage.authTier, pageCount, // sort_order reflects page definition order in the template
            tplPage.navLabel ?? null, input.slug);
            ...
            const pageIdRows = (await db.$queryRawUnsafe(`SELECT id FROM app_pages WHERE slug = $1 LIMIT 1;`, tplPage.slug));
            ...
            await db.$executeRawUnsafe(`DELETE FROM page_sections WHERE page_id = $1;`, pageId);
            ...
            for (let i = 0; i < tplPage.blockTypes.length; i++) {
                ...
                await db.$executeRawUnsafe(`INSERT INTO page_sections (id, page_id, sort_order, block_type, config)
           VALUES ($1, $2, $3, CAST($4 AS "BlockType"), CAST($5 AS jsonb));`, sectionId, pageId, i, blockType, JSON.stringify({ minTier: tplPage.authTier }));
            }
            pageCount++;
    ...
    try {
        ...
        console.warn(`[tenant-seed] Could not clear navigation_items:`, err.message);
    }
    for (let i = 0; i < template.defaultNavItems.length; i++) {
        ...
        try {
            ...
            await db.$executeRawUnsafe(`INSERT INTO navigation_items (id, title, path, icon, auth_tier, sort_order, tenant_slug, created_at, updated_at)
         VALUES ($1, $2, $3, $4, CAST($5 AS "AuthTier"), $6, $7, NOW(), NOW());`, navId, navItem.title, navItem.path, navItem.icon, navItem.authTier, i, // sort_order reflects nav definition order in the template
            input.slug);
            navCount++;
        }
        catch (err) {
 ...
 */
export async function seedTemplateSecurityGroups(db, templateId) {
    const groups = [
        {
            code: 'platform-admin',
            name: 'Platform Admin',
    ...
    for (const g of groups) {
        try {
            ...
            await db.$executeRawUnsafe(`INSERT INTO security_groups (id, code, name, description, is_system, permissions, created_at)
         VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, NOW())
         ON CONFLICT (code) DO UPDATE
           SET name = $2, description = $3, is_system = $4, permissions = $5;`, g.code, g.name, g.description, g.isSystem, g.permissions);
            count++;
        }
        catch (err) {
 ...
 */
export async function seedTemplateBranding(slug, db, input) {
    try {
        await db.$executeRawUnsafe(`UPDATE app_settings
       SET brand_primary_color = $1, brand_secondary_color = $2, updated_at = NOW()
       WHERE id = $3;`, input.primaryColor, input.secondaryColor, slug);
