 ...
 */
export async function linkProject(outputDir, slug, token) {
 ...
 */
export async function injectEnvVars(outputDir, envVars, token) {
    const flag = token ? `--token=${token}` : '';
    let injected = 0;
    let skipped = 0;
    ...
    console.log(`[vercel-cli] Env vars: ${injected} added, ${skipped} skipped/already-existed`);
}
// ── Deploy trigger ────────────────────────────────────────────────
/**
 ...
 */
export async function triggerDeploy(outputDir, token) {
    const flag = token ? `--token=${token}` : '';
 ...
 */
export async function deployViaCli(outputDir, slug, envVars) {
    const token = process.env.VERCEL_TOKEN ?? '';
    // 1. Link project
...
function readProjectId(outputDir) {
 ...
 */
function runCli(cmd, cwd, options = {}) {
    try {
        ...
        throw new Error(`Vercel CLI command failed: ${stderr.slice(0, 200) || 'unknown error'}`);
    }
}
