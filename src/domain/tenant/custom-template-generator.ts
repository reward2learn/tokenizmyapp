...
function buildSystemPrompt(): string {
  return [
    ...
    '',
    'Web3 wallet (provider: Reown AppKit): enable by default (enabled=true) so tenants get social wallet sign-in and crypto billing. Disable only when the business clearly has no use for wallets or on-chain payments.',
    'When enabled, prefer connectMode "social" so users sign in with a familiar identity instead of managing a seed phrase.',
    ...
    'Always return a complete web3Wallet object.',
    '',
