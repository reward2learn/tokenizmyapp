...
function getConfig() {
    ...
    if (!apiKey) {
        throw new Error('NEON_API_KEY is not set. Get it from https://console.neon.tech → Settings → API Keys');
    }
    if (!projectId) {
        throw new Error('NEON_PROJECT_ID is not set. Find it in the Neon console project URL or dashboard.');
    }
...
function toPostgresIdentifier(slug) {
...
function derivePooledUrl(directUrl) {
...
async function neonFetch(path, options = {}, maxRetries = 4) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        if (!res.ok) {
            ...
            console.warn(`[neon-provision] Response ${res.status} for ${options.method || 'GET'} ${path}: ${errBody.slice(0, 300)}`);
        }
        ...
        if (res.status === 423) {
            ...
            console.warn(`[neon-provision] Project locked (423), retrying in ${delay}ms ` +
        ...
        console.warn(`[neon-provision] Rate limited (429), retrying in ${delay}ms ` +
            `(attempt ${attempt + 1}/${maxRetries + 1})`);
        await sleep(delay);
    }
...
async function findBranchIdByName(name) {
...
async function listDatabases(branchId) {
    const { projectId } = getConfig();
...
async function databaseExists(branchId, dbName) {
...
export async function getMainBranchId() {
    const branches = await listBranches();
    const main = branches.find((b) => b.primary) ??
        branches.find((b) => b.is_default) ??
        branches.find((b) => b.name === 'main');
    if (!main) {
        throw new Error('No main/primary branch found in Neon project. ' +
 ...
 */
export async function provisionTenantDatabase(slug) {
    const { projectId } = getConfig();
    ...
    try {
        ...
        if (createRes.status === 409) {
            ...
            throw new Error(`Failed to create Neon branch "${branchName}": ${createRes.status} ${err.slice(0, 200)}`);
        }
        else {
    ...
    if (dbExists) {
        ...
        const dbRes = await neonFetch(`/projects/${projectId}/branches/${branchId}/databases`, {
            method: 'POST',
        ...
        if (!dbRes.ok && dbRes.status !== 409) {
            ...
            throw new Error(`Failed to create database "${databaseName}" in branch ${branchId}: ` +
    ...
    console.log(`[neon-provision] Provisioned ${branchName}: branch=${branchId}, ` +
 ...
 */
export async function deprovisionTenantDatabase(slug) {
    const { projectId } = getConfig();
    const branchName = `tenant-${slug}`;
    let branchId;
    try {
    ...
    if (!res.ok && res.status !== 404) {
        const err = await res.text().catch(() => '');
        throw new Error(`Failed to delete Neon branch "${branchName}" (${branchId}): ` +
 ...
 * immediately ready.
 */
async function getConnectionStrings(branchId, databaseName) {
    const { projectId } = getConfig();
    const maxAttempts = 5;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const res = await neonFetch(`/projects/${projectId}/branches/${branchId}/endpoints`);
        if (res.ok) {
            const data = await res.json();
        ...
        if (attempt < maxAttempts - 1) {
            console.log(`[neon-provision] Endpoints not ready for branch ${branchId}, ` +
                `retrying (${attempt + 1}/${maxAttempts})...`);
            await sleep(2000);
        }
    }
    throw new Error(`Could not retrieve connection strings for branch ${branchId} ` +
 ...
 */
function derivePooledHost(directHost) {
