...
export function parseExpenseHeuristic(text, department) {
    ...
    const totalMatch = normalized.match(/(?:grand\s*)?total|jumlah|amount\s*due|total\s*bayar|net\s*amount|sub\s*total/i);
    let totalVal = null;
    if (totalMatch) {
        ...
        const amounts = [...slice.matchAll(/(\d{1,3}(?:[.,]\d{3})+|\d{4,})/g)]
            ...
            .filter((n) => n != null && n > 0);
        if (amounts.length)
            totalVal = amounts[amounts.length - 1];
    }
...
export async function parseExpenseWithAi(text, department, apiKey) {
    ...
    if (!raw)
        throw new Error('Empty AI parse response');
