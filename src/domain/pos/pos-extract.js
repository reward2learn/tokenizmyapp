...
function assignPair(raw, prefix, pair) {
    if (pair.qty != null)
        raw[`${prefix}_qty`] = pair.qty;
    if (pair.amount != null)
        raw[`${prefix}_amount`] = pair.amount;
}
export function parsePosTextHeuristic(rawText) {
    ...
    if (beginRcpt)
        raw.begin_receipt_no = beginRcpt[1];
    const endRcpt = flat.match(/End\s*Receipt[#i]?\s*([A-Z]\d+)/i);
    if (endRcpt)
        raw.end_receipt_no = endRcpt[1];
    ...
    let confidence = 'low';
    if (raw.nett_sales && raw.total_covers && raw.report_date)
        confidence = 'high';
    else if (raw.nett_sales || raw.total_covers)
        confidence = 'medium';
...
export function sanitizeExtraction(raw) {
    ...
    for (const key of Z_REPORT_FIELD_KEYS) {
        if (raw[key] !== undefined && raw[key] !== null && raw[key] !== '') {
            if (key === 'report_date')
                out[key] = String(raw[key]).slice(0, 10);
            else if (key === 'report_time')
                out[key] = String(raw[key]).slice(0, 8);
            ...
            else if (key.endsWith('_amount') || key === 'total_sales' || key === 'estimated_sales'
    ...
    if (!out.report_date && raw.date)
        out.report_date = raw.date.slice(0, 10);
    if (!out.nett_sales && raw.revenue)
        out.nett_sales = toInt(raw.revenue);
    if (!out.total_sales && raw.revenue)
        out.total_sales = toInt(raw.revenue);
    if (!out.total_covers && out.item_sales_qty)
        out.total_covers = out.item_sales_qty;
    if (!out.total_covers && raw.guests_count)
        out.total_covers = toInt(raw.guests_count);
    if (!out.avg_covers && raw.avg_spend)
        out.avg_covers = toInt(raw.avg_spend);
    if (!out.gofood_amount && raw.gofood_revenue)
        out.gofood_amount = toInt(raw.gofood_revenue);
    if (!out.dine_in_amount && raw.dine_in_revenue)
        out.dine_in_amount = toInt(raw.dine_in_revenue);
    if (!out.total_bills && raw.bills_count)
        out.total_bills = toInt(raw.bills_count);
    if (!out.total_item_discount_amount && raw.discounts)
        out.total_item_discount_amount = toInt(raw.discounts);
    if (!out.pre_send_void_amount && raw.voids)
        out.pre_send_void_amount = toInt(raw.voids);
