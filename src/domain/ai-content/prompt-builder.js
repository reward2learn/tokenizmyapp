...
function fmtIdr(n) {
    ...
    if (n >= 1_000_000)
        return `IDR ${(n / 1_000_000).toLocaleString('id-ID')}`;
    if (n >= 1_000)
...
// ── Section builders ────────────────────────────────────
function buildCompanyInfoSection(data) {
...
function buildProfitAndLossSection(lines) {
    if (!lines.length)
        return '';
    const tableRows = lines.map((l) => `| ${l.accountCode} | ${l.description} | ${fmtIdrExact(l.amount)} |`);
    return [
...
function buildBepSection(rows) {
    if (!rows.length)
        return '';
    const tableRows = rows.map((r) => `| ${r.period} | ${fmtIdrExact(r.totalRevenue)} | ${fmtIdrExact(r.totalCos)} | ${fmtPct(r.grossMarginPct)} | ${fmtIdrExact(r.totalFixedCost)} | ${fmtIdrExact(r.bepRevenue)} | ${r.bepCoverage.toFixed(2)}x |`);
    return [
...
function buildMonthOnMonthSection(lines) {
    if (!lines.length)
        return '';
    const tableRows = lines.map((l) => `| ${l.description} | ${fmtIdrExact(l.previousMonth)} | ${fmtIdrExact(l.currentMonth)} | ${(l.changePct * 100).toFixed(1)}% |`);
    return [
...
function buildMonthlyVarianceSection(data) {
    const rows = data.monthlyVariance;
    if (!rows.length)
        return '';
    const tableRows = rows.map((r) => `| ${r.item} | ${fmtIdrExact(r.mayValue)} | ${fmtIdrExact(r.juneValue)} | ${fmtIdrExact(r.variance)} | ${(r.variancePct * 100).toFixed(1)}% |`);
    return [
...
function buildDailySalesSection(data) {
    ...
    return [
        ...
        ds.terraceRevenue.length ? ds.terraceRevenue.map((r) => `- **${r.description}**: ${fmtIdrExact(Object.values(r.dailyValues).reduce((a, b) => a + b, 0))}`).join('\n') : '- No detailed data',
        ``,
        `### Club Revenue`,
        ds.clubRevenue.length ? ds.clubRevenue.map((r) => `- **${r.description}**: ${fmtIdrExact(Object.values(r.dailyValues).reduce((a, b) => a + b, 0))}`).join('\n') : '- No detailed data',
        ``,
        `### Spend per Guest (daily average): ${fmtIdrExact(Math.round(avgSpendValue))}`,
        ``,
    ].join('\n');
}
function buildSummaryPlSection(data) {
    if (!data.summaryPl.length)
        return '';
    const sections = data.summaryPl.map((year) => {
        const lines = year.lines.map((l) => `| ${l.description} | ${fmtIdrExact(l.amount)} |`);
        return [
            `### ${year.year}`,
            `| Description | Amount |
|-------------|--------|
...
function buildBalanceSheetSection(data) {
    if (!data.balanceSheet.length)
        return '';
    const rows = data.balanceSheet.map((l) => `| ${l.description} | ${fmtIdrExact(l.amount)} |`);
    return [
...
// ── Main prompt builder ─────────────────────────────────
export function buildGenerationPrompt(data, additionalContext) {
    const tenant = getTenantConfig();
 ...
 */
export function buildDashboardPrompt(data, additionalContext) {
    const tenant = getTenantConfig();
    const businessName = data.company || tenant.displayName;
    const sections = [
