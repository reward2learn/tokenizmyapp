 ...
 */
export async function rephraseReviewDocumentsFromChat(db, messages, summary) {
 ...
 */
export async function rephraseExecutiveSummary(db, messages, summary) {
    const apiKey = await resolveOpenAiKey();
    if (!apiKey) {
        return { success: false, partsUpdated: 0, error: 'OpenAI API key not configured.' };
    }
 ...
 */
async function callForDocument(prompt, documentType, apiKey, model) {
    try {
