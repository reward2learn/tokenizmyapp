 ...
 */
async function callOpenAiForDocument(prompt, apiKey, documentType, model = 'gpt-4o', onProgress) {
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            throw new Error('AI response was not valid JSON. Raw response: ' + reply.slice(0, 500));
        }
    }
...
// ── DB save helpers ─────────────────────────────────────
async function saveExecutiveSummary(db, markdown) {
    try {
...
async function saveBusinessReviewParts(db, parts, onProgress) {
    const saved = [];
    for (let i = 0; i < parts.length; i++) {
        ...
        try {
            ...
            console.error(`[ai-content] Failed to save review part ${part.slug}:`, err);
        }
        const pctBase = 75;
 ...
 */
export async function generateAndSave(db, onProgress, source, model, additionalContext, overridePrompt) {
    try {
        ...
        if (!apiKey) {
            const errorMsg = 'OpenAI API key not configured. Set it in Config > OpenAI Key or via OPENAI_API_KEY env var.';
            onProgress?.({
                step: 'error',
                message: errorMsg,
                pct: 0,
        ...
        const businessReview = await callOpenAiForDocument(prompt, apiKey, 'businessReview', model, onProgress);
        ...
        const executiveSummary = await callOpenAiForDocument(prompt, apiKey, 'executiveSummary', model, onProgress);
        const content = {
            businessReview,
            executiveSummary,
        ...
        if (savedParts.length > 0) {
            ...
            setDynamicReviewParts(parts.map((p) => ({
                partSlug: p.slug,
                partKey: p.partKey,
                title: p.title,
                authTier: 'google',
        ...
        const execSummarySaved = await saveExecutiveSummary(db, content.executiveSummary);
        // ── 8. Complete ─────────────────────────────────────
        const result = {
            success: true,
            prompt,
