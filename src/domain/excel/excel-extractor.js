 ...
 */
function detectHeaderRow(ws, searchLabels = ['DESCRIPTION']) {
...
function extractPl(ws) {
    ...
    for (const row of rows) {
        ...
        const d = cells[3];
        if (b === '4-0000' && c === 'INCOME') {
            inIncome = true;
            continue;
        }
        if (b === '5-0000' && c === 'Cost Of Sales') {
            inIncome = false;
            inCos = true;
            continue;
        }
        if (b === '6-0000' && c === 'EXPENSES') {
            inCos = false;
            continue;
        }
        if (b === '4-9999') {
...
function extractMonthlyVariance(ws) {
    const rows = [];
...
function extractBepMonthly(ws) {
    ...
    for (const row of rows) {
        ...
        else if (label === 'Total Payroll') {
            totalPayroll = vals;
        }
        else if (label === 'Other Fixed Cost') {
            otherFixedCost = vals;
        }
        else if (label === 'Gross Margin %') {
            grossMarginPct = vals;
        }
        else if (label === 'Total Fixed Cost') {
            totalFixedCost = vals;
        }
        else if (label === 'BEP Revenue') {
            bepRevenue = vals;
        }
        else if (label === 'BEP Coverage ') {
            bepCoverage = vals;
        }
    }
    ...
    for (let c = 1; c < headerRow.length; c++) {
        ...
        if (h) {
            ...
            if (ds)
                periods.push(periodFromDateStr(ds));
            else
                periods.push(String(h).trim());
        }
        else {
            periods.push('');
        }
    ...
    // Build BEP rows
    const maxLen = Math.max(totalRevenue.length, totalCos.length, totalPayroll.length, otherFixedCost.length, grossMarginPct.length, totalFixedCost.length, bepRevenue.length, bepCoverage.length);
...
function extractDailySales(ws) {
    const rows = utils.sheet_to_json(ws, { header: 1, defval: '' });
    const terraceRevenue = [];
    const clubRevenue = [];
    const totals = {};
    const spendPerGuest = {};
    let inTerrace = false;
    ...
    for (const row of rows) {
        ...
        if (b === 'Terrace Revenue:') {
            currentSection = 'terrace';
            inTerrace = true;
            inClub = false;
            continue;
        }
        ...
        if (currentSection && b && r[2] !== undefined) {
            ...
            if (currentSection === 'terrace')
                terraceRevenue.push(rowData);
            else
                clubRevenue.push(rowData);
        }
    }
    return { terraceRevenue, clubRevenue, totals, spendPerGuest };
}
function extractMonthOnMonth(ws) {
    const lines = [];
    const rows = utils.sheet_to_json(ws, { header: 1, defval: '' });
    const headerRow = detectHeaderRow(ws, ['DESCRIPTION']);
    for (let i = headerRow + 1; i < rows.length; i++) {
...
function extractSummaryPl(ws) {
    ...
    for (const yc of yearCols) {
        ...
        for (let r = headerRowIdx + 2; r < rows.length; r++) {
            ...
            if (!desc || row[yc.colIdx] === undefined)
                continue;
 ...
 */
export function extractExcelData(source) {
    let buf;
    ...
    if (Buffer.isBuffer(source)) {
        ...
        if (!existsSync(diskPath)) {
            throw new Error('Workbook file not found on disk and no data buffer provided. ' +
 ...
 */
export function extractExcelDataFromBuffers(sources) {
    const all = sources.map((buf) => extractExcelData(buf));
    if (all.length === 0) {
