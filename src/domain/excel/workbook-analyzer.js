...
function detectCategory(tabName) {
...
// ── Column analysis ─────────────────────────────────────
function guessDataType(label, sampleValues) {
    const labelLower = label.toLowerCase();
    if (/amount|total|sales|revenue|income|cost|expense|balance|value|price|sum/i.test(labelLower)) {
        return 'idr';
    }
    if (/pct|margin|rate|%/i.test(labelLower))
        return 'pct';
    if (/date|period|month|year/i.test(labelLower))
        return 'date';
    if (/qty|count|number|no\.|covers|bills|guests|staff/i.test(labelLower))
        return 'number';
    // Check sample values
    const numericSamples = sampleValues.filter((v) => typeof v === 'number' || (typeof v === 'string' && /^[\d,.\-]+$/.test(v.trim())));
...
function findHeaderRow(ws) {
    ...
    for (let i = 0; i < maxScan; i++) {
        ...
        if (nonEmptyCount <= 2 && TITLE_KEYWORDS.test(firstCell))
            continue;
...
function extractPeriods(headers, sampleValues) {
    const periods = [];
    for (const h of headers) {
        const hStr = String(h ?? '');
        // Match "YYYY-MM-DD" or "YYYY-MM" or "MM/DD/YYYY" or "Jan 2026" etc.
        const dateMatch = hStr.match(/(\d{4})[-/](\d{1,2})(?:[-/]\d{1,2})?|(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{4})/i);
        if (dateMatch) {
            periods.push(hStr);
        }
    }
...
function extractSampleRows(ws, headerRow, maxSamples = 5) {
    const json = utils.sheet_to_json(ws, {
        defval: '',
        header: 1,
    });
    const dataRows = json.slice(headerRow).filter((row) => row.some((c) => c !== '' && c !== undefined && c !== null));
    return dataRows.slice(0, maxSamples).map((row) => {
        const obj = {};
        row.forEach((val, idx) => {
            obj[`col_${idx + 1}`] = val;
        });
...
// ── Sheet analysis ──────────────────────────────────────
function analyzeSheet(ws, tabName) {
    ...
    const dataRows = allRows.slice(headerRow).filter((r) => r.some((c) => c !== '' && c !== undefined && c !== null));
    const columnCount = Math.max(...dataRows.map((r) => r.length), headers.length, 1);
    ...
    const hasFinancialData = columns.some((c) => c.dataType === 'idr' || c.dataType === 'amount');
...
function detectHeaderInfo(ws) {
    ...
    for (let i = 0; i < Math.min(rows.length, 6); i++) {
        ...
        const periodMatch = text.match(/(?:periode|month|period|per)\s*(?:\sof\s+)?(\w+\s+\d{4})/i);
        if (periodMatch)
            period = periodMatch[1];
        // Also try "MONTH OF <text>"
        const monthOfMatch = text.match(/MONTH OF\s+(.+)/i);
        if (monthOfMatch && !period)
            period = monthOfMatch[1].trim();
    }
    return { company, period };
}
// ── Main analyzer ───────────────────────────────────────
export function analyzeWorkbook(data, fileName = 'workbook.xlsx') {
...
// ── Summary builder ─────────────────────────────────────
function buildSummary(sheets, company, period, fileName) {
    const parts = [
        `Workbook: ${fileName}`,
    ...
    for (const s of sheets) {
        ...
        parts.push(`  - ${s.tabName} → "${s.title}"${financialTag}${multiYearTag}${periodTag}`);
        parts.push(`    ${s.columns.length} columns, ${s.rowCount} data rows`);
    }
    // Use-case generation
    parts.push('');
 ...
 */
export function generatePagesFromAnalysis(analysis) {
    const pages = [];
 ...
 */
export function generateSheetMarkdown(sheet) {
    const lines = [
...
export function generateAnalysisMarkdown(analysis) {
    const lines = [
