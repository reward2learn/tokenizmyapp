...
function getStaffCost(sheet, col, sheetName) {
    const costRows = sheetName === 'RedRuby' ? [47, 49, 51, 53, 55, 57, 59, 61, 63, 65, 66] : [35, 37, 39, 41, 43, 44];
    let total = 0;
...
function extractPnlLines(sheet, col, sheetName) {
    ...
    for (const item of PNL_LINE_ITEMS) {
        ...
        const value = raw != null && typeof raw === 'number' && !Number.isNaN(raw) ? raw : null;
        lines.push({
            key: item.key,
            label: item.label,
            value,
...
export function parseFinancialProjectionsFromBuffer(data) {
    const wb = read(data, { type: 'buffer' });
    return parseWorkbook(wb);
}
export function parseFinancialProjectionsFromExcel(excelPath) {
