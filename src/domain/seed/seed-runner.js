...
import { parseFinancialProjectionsFromBuffer, } from '@/domain/seed/financial-excel';
import { getSourceDir, getWebsiteRoot, PRIVACY_HTML_PATH, readSourceFile, readSourceText, sourceFileExists, TERMS_HTML_PATH, writeSourceFile, } from '@/domain/seed/source-files';
import { BUSINESS_NAME, CURRENT_METRICS, FIVE_LEVERS, KEY_RISKS, LOCATION, MONTHLY_TARGETS, PRIORITY_ACTIONS, SITUATION_SUMMARY, STRATEGIC_PARTNERSHIPS, TARGET_METRICS, TASK_PLAYBOOK, } from '../../../lib/knowledge-base.js';
import { analyzeWorkbook, generatePagesFromAnalysis, generateAnalysisMarkdown, generateSheetMarkdown, } from '@/domain/excel/workbook-analyzer';
...
function buildKnowledgeSnippets(executiveSummaryMd, termsMd, privacyMd) {
    return [
        ...
        },
        {
            key: 'five_levers',
            category: 'strategy',
            content: FIVE_LEVERS.map((l) => `${l.num}. ${l.name} — ${l.impact}\nTarget: ${l.target}\nActions:\n${l.actions.map((a) => `  - ${a}`).join('\n')}`).join('\n\n'),
        },
        {
        ...
        },
        {
            ...
            category: 'metrics',
            content: MONTHLY_TARGETS.map((t) => `${t.month}: revenue ${t.revenue}, ebitda ${t.ebitda}, guests ${t.guests}/day, spend ${t.spend}, staff ${t.staffPct}%`).join('\n'),
        },
 ...
 */
const KNOWN_ROLES = PERSONS.filter((p) => p.sub !== 'admin' || true).map((p) => {
    ...
    const legacyCode = p.sub === 'admin' ? 'Admin' :
        p.sub === 'ama' ? 'Ama' :
            p.sub === 'graham' ? 'Graham' :
                p.sub === 'james' ? 'James' :
 ...
 */
function resolveOwnerCodes(ownerCodes) {
    if (ownerCodes.length === 0)
        return [...INDIVIDUAL_ROLE_CODES];
    return ownerCodes;
}
function buildTasks() {
    const tasks = [];
    const push = (labels, priority, dueOffsetDays) => {
 ...
 */
export async function seedTaskTracking(prisma) {
...
async function upsertFinancialProjectionRaw(prisma, row) {
...
function resolveSources(options) {
    ...
    const excel = overrides.excel ??
        (sourceFileExists('excel', sourceDir)
            ? readSourceFile('excel', sourceDir)
            : undefined);
    const businessReview = overrides.businessReview ??
    ...
    if (excel === undefined && businessReview === undefined && executiveSummary === undefined) {
        throw new Error('No sources found — upload at least the Business Review (.md) or Executive Summary (.md). ' +
...
export async function seedFromSources(options = {}) {
    ...
    const excelBuffer = Array.isArray(excel) ? excel[0] : excel;
    const excelBuffers = Array.isArray(excel) ? excel : (excel ? [excel] : []);
    ...
    if (allAnalyses.length === 1) {
        ...
        workbookAnalysisMd = generateAnalysisMarkdown(combined);
    }
    ...
    if (reviewParts.length > 0) {
        setDynamicReviewParts(reviewParts.map((p) => ({
            partSlug: p.slug,
            partKey: p.partKey,
            title: p.title,
            authTier: 'google',
    ...
    const allExcelBuffers = Array.isArray(excel) ? excel : (excel ? [excel] : []);
    ...
    try {
        ...
        for (const part of reviewParts) {
            ...
            const authTier = (catalog?.authTier ?? 'google');
