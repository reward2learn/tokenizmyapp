...
export const provisionTenant = inngest.createFunction({ id: 'provision-tenant', retries: 3, triggers: [{ event: 'tenant.created' }] }, async ({ event, step }) => {
    ...
    await step.run('update-tenant-record', async () => {
        const { createRawClient } = await import('@/lib/db');
        const db = createRawClient();
        await db.$executeRawUnsafe(`UPDATE tenants SET status = 'live', vercel_project_id = $1, app_url = $2, db_url = $3, updated_at = CURRENT_TIMESTAMP WHERE slug = $4;`, deployResult.projectId, deployResult.appUrl, dbInfo?.pooledUrl ?? null, slug);
        return true;
    });
