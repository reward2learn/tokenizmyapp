...
function extractTitle(html) {
    ...
    if (ogTitleMatch)
        return decodeHtml(ogTitleMatch[1].trim());
    return '';
}
function extractMetaDescription(html) {
    ...
    if (ogDescMatch)
        return decodeHtml(ogDescMatch[1].trim());
    return '';
}
function extractBusinessName(html, title, hostname) {
    // Try JSON-LD
    ...
    if (siteNameMatch)
        return decodeHtml(siteNameMatch[1].trim());
...
function extractInstagramJson(html) {
 ...
 */
async function extractColorsFromImageBase64(base64Data) {
 ...
 */
export function recommendTemplate(scraped) {
    ...
    // URL-based hints
    if (url.includes('restaurant') || url.includes('cafe') || url.includes('bar'))
        scores['restaurant'] += 5;
    if (url.includes('hotel') || url.includes('resort'))
        scores['hotel'] += 5;
    if (url.includes('shop') || url.includes('store'))
        scores['ecommerce-retail'] += 5;
    if (url.includes('clinic') || url.includes('hospital'))
        scores['healthcare'] += 5;
 ...
 */
export function generatePromptFromScraped(scraped) {
    const parts = [];
