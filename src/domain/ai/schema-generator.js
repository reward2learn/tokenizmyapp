...
import { schemaGenerationZodSchema } from './schema-generation-schema';
...
export async function generateSchemaFromPrompt(userPrompt, templateId) {
...
export function mockGenerateSchema(templateId) {
    const schemaOrgType = SCHEMA_ORG_TYPES[templateId] ?? 'LocalBusiness';
    const w3cStandard = W3C_STANDARDS[templateId] ?? 'schema.org';
