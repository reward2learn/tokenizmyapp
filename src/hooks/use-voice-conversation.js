...
import { getSpeechRecognitionCtor, } from '@/lib/chat/speech-recognition';
import { transcriptFromResultOffset, VOICE_DEBOUNCE_MS } from '@/lib/chat/voice-transcript';
import { checkMicrophoneAccess, isBenignRecognitionError, microphoneErrorMessage, } from '@/lib/chat/check-microphone';
...
function extractSpeakablePrefix(text, isFinal) {
    if (isFinal)
        return text;
    if (text.trim().length < STREAMING_TTS_MIN_CHARS)
        return '';
    ...
    if (text.length < STREAMING_TTS_SOFT_LIMIT)
        return '';
    const searchWindow = text.slice(0, STREAMING_TTS_SOFT_LIMIT);
    const fallbackEnd = Math.max(searchWindow.lastIndexOf(', '), searchWindow.lastIndexOf('; '), searchWindow.lastIndexOf(': '), searchWindow.lastIndexOf(' '));
    return fallbackEnd >= STREAMING_TTS_MIN_CHARS ? text.slice(0, fallbackEnd + 1) : '';
}
export function useVoiceConversation({ isStreaming, lastAssistantText, onTranscriptChange, onSend, synthesizeVoice, }) {
    ...
    const [micUnavailableMessage, setMicUnavailableMessage] = useState(null);
    ...
    const playbackSettleRef = useRef(null);
    ...
    const flushTranscript = useCallback(async () => {
        ...
        if (!trimmed || voicePausedRef.current || isSendingRef.current)
            return;
    ...
    const handleMicUnavailable = useCallback((message) => {
        ...
        recognition.onend = () => {
            ...
            if (sessionDuration < RAPID_RESTART_WINDOW_MS) {
                ...
                if (rapidRestartCountRef.current >= MAX_RAPID_RESTARTS) {
                    handleMicUnavailable('Voice input keeps stopping. Check your microphone and browser permissions, then try again.');
                    return;
                }
            }
            else {
    ...
    const playSpeechChunk = useCallback(async (text) => {
    ...
    const processSpeechQueue = useCallback(async () => {
        if (speechQueueRunningRef.current || voicePausedRef.current || assistantMutedRef.current)
            return;
        ...
        try {
            while (speechQueueRef.current.length) {
                ...
                if (!nextText)
                    continue;
                if (voicePausedRef.current || assistantMutedRef.current)
                    break;
                const ok = await playSpeechChunk(nextText);
                if (!ok && ttsAbortRef.current === null)
                    break;
            }
        }
        finally {
    ...
    const queueSpeech = useCallback((text) => {
        const trimmed = text.trim();
        if (!trimmed || voicePausedRef.current || assistantMutedRef.current)
            return;
    ...
    const setAssistantMutedValue = useCallback((muted) => {
    ...
    const setAssistantVolume = useCallback((volume) => {
    ...
    useEffect(() => {
        if (!voiceMode || !lastAssistantText?.trim())
            return;
