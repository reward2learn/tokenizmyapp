...
vi.mock('@/components/charts/financial-chart', () => ({
    FinancialChart: () => _jsx("div", { "data-testid": "financial-chart-mock" }),
}));
function makeTestStore() {
    ...
    store.dispatch(financialApi.util.upsertQueryData('getChartOverview', 'conservative', mockOverview));
    return store;
}
describe('ChartFinancialBlock', () => {
    ...
    it('renders KPI filter buttons and chart', () => {
        ...
        render(_jsx(Provider, { store: store, children: _jsx(ChartFinancialBlock, { config: { variant: 'dashboard', minTier: 'public' } }) }));
    ...
    it('switches KPI when filter button clicked', () => {
        const store = makeTestStore();
        render(_jsx(Provider, { store: store, children: _jsx(ChartFinancialBlock, { config: { variant: 'dashboard' } }) }));
