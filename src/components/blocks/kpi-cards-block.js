...
import { setChartKpi } from '@/store/ui-slice';
const OPS_KPIS = [
...
function getValAtIndex(actual, forecast, idx) {
    const a = actual?.[idx];
    if (a != null)
        return a;
...
export function KpiCardsBlock({ config }) {
    const { variant } = parseBlockConfig('kpi_cards', config);
    if (variant !== 'ops') {
        return null;
    }
    return (_jsx(Suspense, { fallback: _jsx(Box, { sx: { display: 'flex', justifyContent: 'center', py: 3 }, children: _jsx(CircularProgress, { size: 24 }) }), children: _jsx(KpiCardsBlockInner, { config: config }) }));
}
function KpiCardsBlockInner({ config }) {
    const { variant } = parseBlockConfig('kpi_cards', config);
    ...
    const monthIndex = useMemo(() => resolveMonthIndex(overview?.labels ?? [], selectedMonthLabel), [overview?.labels, selectedMonthLabel]);
    ...
    return (_jsx(Box, { component: "section", sx: { maxWidth: 900, mx: 'auto', px: 3, pt: 3 }, children: _jsx(Box, { sx: {
                display: 'grid',
                gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(5, 1fr)' },
                gap: 1.75,
            }, children: cards.map((c) => (_jsx(MetricCard, { label: c.kpiLabel, value: c.display, change: c.change, active: chartKpi === c.key, onClick: () => dispatch(setChartKpi(c.key)) }, c.key))) }) }));
}
