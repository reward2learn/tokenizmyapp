...
export function ActionChecklistBlock({ config }) {
    ...
    if (!phases)
        return null;
    return (_jsxs(Box, { component: "section", sx: { mx: 'auto', px: 3, py: 4 }, children: [_jsx(Typography, { variant: "h5", component: "h2", sx: { fontWeight: 800, textAlign: 'center', mb: 1 }, children: "Step-by-Step Action Plan" }), _jsx(Typography, { variant: "body2", color: "text.secondary", sx: { textAlign: 'center', mb: 3, maxWidth: 520, mx: 'auto' }, children: "Three phases from survival to sustainable profitability. Click each phase to expand." }), phases.map((phase) => (_jsxs(Accordion, { expanded: expanded === phase.id, onChange: (_, isExpanded) => setExpanded(isExpanded ? phase.id : false), elevation: 0, sx: {
                    mb: 1.5,
                    bgcolor: 'rgba(255,255,255,0.03)',
                    border: '1px solid',
                    borderColor: 'divider',
                    '&:before': { display: 'none' },
                }, children: [_jsxs(AccordionSummary, { expandIcon: _jsx(ExpandMoreIcon, { sx: { color: 'primary.main' } }), children: [_jsx(Box, { sx: { flex: 1 }, children: _jsxs(Typography, { variant: "subtitle1", sx: { fontWeight: 600 }, children: [phase.title, ' ', _jsxs(Typography, { component: "span", variant: "body2", color: "text.secondary", children: ["\u2014 ", phase.period] })] }) }), _jsx(Chip, { label: phase.impact, size: "small", color: "primary", variant: "outlined", sx: { ml: 1 } })] }), _jsx(AccordionDetails, { children: phase.actions.map((action) => (_jsxs(Typography, { variant: "body2", color: "text.secondary", sx: { mb: 0.75 }, children: ["\u2022 ", action] }, action))) })] }, phase.id)))] }));
}
