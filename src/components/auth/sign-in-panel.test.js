...
function renderPanel() {
    ...
    return render(_jsx(Provider, { store: store, children: _jsx(SignInPanel, { requiredTier: "pin" }) }));
}
describe('SignInPanel', () => {
    beforeEach(() => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockFetchResponse({ ok: true, success: true })));
    });
    afterEach(() => {
        cleanup();
        vi.unstubAllGlobals();
