...
import { axisTickCallback, findCurrentMonthIndex, formatChartValue, } from '@/lib/chart-utils';
let chartJsRegistered = false;
function buildBarSeries(labels, actual, forecast) {
    const barData = [];
    const barColors = [];
    const cumulative = [];
    let running = 0;
...
export function FinancialChart({ kpi, labels, actual, forecast, scenarioForecasts, variant = 'dashboard', height = 300, selectedIndex = null, onMonthClick, isLoading = false, isError = false, }) {
    ...
    const { barData, barColors, cumulative } = useMemo(() => buildBarSeries(labels, actual, forecast), [labels, actual, forecast]);
        ...
        scales: {
            ...
            y: {
                ...
                ticks: {
                    color: '#8888a0',
                    font: { size: 9 },
                    callback: (val) => axisTickCallback(kpi, val),
                },
            },
            y1: {
                position: 'right',
    ...
    if (isLoading) {
        return (_jsx(Box, { sx: { display: 'flex', alignItems: 'center', justifyContent: 'center', height }, children: _jsx(CircularProgress, { size: 32, color: "primary" }) }));
    }
    if (isError) {
        return (_jsx(Typography, { variant: "body2", color: "text.secondary", sx: { textAlign: 'center', pt: 8 }, children: "Chart data unavailable" }));
    }
    if (!labels.length) {
        return (_jsxs(Box, { sx: {
                display: 'flex',
                ...
                gap: 1,
                px: 2,
            }, children: [_jsx(Typography, { variant: "body2", color: "text.secondary", sx: { textAlign: 'center' }, children: "No chart data yet" }), _jsxs(Typography, { variant: "caption", color: "text.secondary", sx: { textAlign: 'center', maxWidth: 360 }, children: ["Run ", _jsx("code", { children: "bun run seed" }), " against production Postgres (see AGENTS.md) to load Excel projections."] })] }));
    }
    return (_jsx(Box, { sx: { height, position: 'relative' }, children: _jsx(Chart, { ref: chartRef, type: "bar", data: chartData, options: options, plugins: [currentMonthPlugin], redraw: selectedIndex != null }) }));
}
