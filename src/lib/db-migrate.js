...
async function withRetry(fn, attempts = 3, baseMs = 500) {
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            ...
            lastErr = err;
            if (!isNeonRetryable(err) || i === attempts - 1)
                throw err;
...
export async function ensureJobQueueTable(db) {
...
export async function ensureConversationsColumns(db) {
...
export async function ensureSecurityTables(db, knownAccounts = []) {
    if (!process.env.POSTGRES_URL) {
    ...
    await withRetry(async () => {
        for (const g of DEFAULT_SECURITY_GROUPS) {
            await raw.$executeRawUnsafe(`INSERT INTO security_groups (id, code, name, description, is_system, permissions)
    ...
    await withRetry(async () => {
        await raw.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS roles (
        id TEXT PRIMARY KEY DEFAULT gen_random_uuid(),
        code TEXT NOT NULL UNIQUE,
        name TEXT NOT NULL,
        email TEXT UNIQUE,
        ...
        for (const fr of FUNCTIONAL_ROLES) {
            ...
            await raw.$executeRawUnsafe(`INSERT INTO roles (id, code, name, is_platform_admin)
         VALUES (gen_random_uuid()::TEXT, $1, $2, $3)
         ON CONFLICT (code) DO UPDATE
           SET name = $2, is_platform_admin = $3;`, fr.code, fr.name, fr.isPlatformAdmin ?? false);
        }
    });
    ...
    await withRetry(async () => {
        await raw.$executeRawUnsafe(`INSERT INTO user_groups (id, user_id, group_id)
...
export async function ensureLegacyTables(db) {
    if (!process.env.POSTGRES_URL) {
