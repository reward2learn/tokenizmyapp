...
let DYNAMIC_PARTS = {};
export function setDynamicReviewParts(parts) {
    DYNAMIC_PARTS = Object.fromEntries(parts.map((p) => [p.partSlug, p]));
}
/**
 ...
 */
export function setDynamicPages(pages) {
...
export function tierAllowsAccess(current, required) {
    return TIER_RANK[current] >= TIER_RANK[required];
}
export function listNavPages(tier, groups = []) {
...
export function resolveReviewPart(partSlug) {
    return getReviewPartCatalog()[partSlug] ?? null;
}
export function listReviewParts() {
...
export function getReviewPartDisplayTitle(title) {
