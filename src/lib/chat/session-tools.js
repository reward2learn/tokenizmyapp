...
export const CHAT_SESSION_ACTIONS = [
    'new_chat_session',
    'clear_conversation',
    'close_conversation',
    'save_conversation',
...
export function isChatSessionAction(value) {
    return typeof value === 'string' && CHAT_SESSION_ACTIONS.includes(value);
}
export function isClientClearSessionAction(action) {
...
export function isExplicitSessionRequest(message) {
...
export async function executeSessionTool(toolName, rawArgs, ctx) {
    let args = {};
    if (rawArgs.trim()) {
        try {
