...
export function classifyAttachment(name, mimeType) {
    if (mimeType.startsWith('image/'))
        return 'image';
    if (SPREADSHEET_MIME.has(mimeType) || SPREADSHEET_EXT.test(name))
        return 'spreadsheet';
    return 'document';
}
export function isSpreadsheet(name, mimeType) {
 ...
 */
export function describeAttachmentForPrompt(attachment) {
