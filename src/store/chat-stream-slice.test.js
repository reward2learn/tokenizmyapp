...
import { addMessage, appendToken, chatStreamSlice, resetStream, setMessages, setStreamError, setStreaming, } from '@/store/chat-stream-slice';
describe('chatStreamSlice', () => {
    ...
    it('records stream errors', () => {
        const state = chatStreamSlice.reducer(chatStreamSlice.reducer(undefined, setStreaming(true)), setStreamError('upstream failed'));
    ...
    it('setMessages replaces the conversation history', () => {
        const state = chatStreamSlice.reducer(undefined, setMessages([
