...
import { attachmentDataUrl, describeAttachmentForPrompt, } from '@/lib/chat/attachments';
import { resolveTtsVoice } from '@/lib/chat/tts-voices';
import { CHAT_SESSION_TOOL_INSTRUCTIONS, } from '@/lib/chat/session-tools';
import { completeChatWithSessionTools, CHAT_WEB_SEARCH_INSTRUCTIONS, } from '@/lib/chat/chat-with-session-tools';
...
let conversationsEnsured = null;
function ensureConversationsOnce() {
...
async function fetchDatabaseContext(db) {
    const parts = [];
    try {
        ...
        if (recent.length) {
            ...
            for (const r of recent) {
                ...
                parts.push(`${date} | ${Number(r.nettSales).toLocaleString()} | ${r.totalCovers} | ${r.avgCovers ? Number(r.avgCovers).toLocaleString() : '-'} | ${r.totalBills || '-'} | ${r.gofoodAmount ? Number(r.gofoodAmount).toLocaleString() : '-'} | ${r.dineInAmount ? Number(r.dineInAmount).toLocaleString() : '-'} | ${r.totCollectionAmount ? Number(r.totCollectionAmount).toLocaleString() : '-'}`);
            }
        }
    }
    catch {
 ...
 */
async function mapReduceContext(fullContext, userMessage, apiKey, model) {
...
async function handleChatPost(request) {
    ...
    try {
        ...
        if (systemMsg && typeof systemMsg.content === 'string' && systemMsg.content.length > 18000) {
            try {
                const reducedContext = await mapReduceContext(systemMsg.content, message, apiKey, 'gpt-4o-mini');
...
async function handleVoicePost(request) {
    let body;
    try {
        body = await request.json();
    }
    ...
    try {
        ...
        if (request.method === 'POST') {
            ...
            try {
                ...
                return NextResponse.json({ success: true, data: { ok: true, id: saved.id, created_at: saved.createdAt } }, { status: 201 });
            }
            catch (err) {
        ...
        if (request.method === 'PATCH') {
            ...
            if (!id || Number.isNaN(numId))
                return legacyError('Invalid id', 400);
            const existing = await db.conversation.findUnique({ where: { id: numId } });
            if (!existing)
                return legacyError('Conversation not found', 404);
...
export async function POST(request) {
    ...
    const resource = url.searchParams.get('resource');
    if (resource === 'voice')
        return handleVoicePost(request);
    if (resource === 'conversations')
        return handleConversations(request, url);
