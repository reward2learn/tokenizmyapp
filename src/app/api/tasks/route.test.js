...
function makeDb(overrides = {}) {
    ...
    return {
        role: {
            findFirst: vi.fn(async ({ where }) => roles.find((r) => r.code === where.code) ?? null),
            findMany: vi.fn(async () => roles),
        },
        task: {
            findMany: vi.fn(async () => tasks),
            findUnique: vi.fn(async ({ where }) => tasks.find((t) => t.id === where.id) ?? null),
...
describe('/api/tasks', () => {
    ...
    it('UC-TASK-01: GET returns role-scoped tasks for a non-admin viewer', async () => {
        ...
        vi.mocked(requireSession).mockResolvedValue({ ok: true, session: session() });
        vi.mocked(requireRead).mockResolvedValue({ ok: true, session: session() });
        const res = await GET(authedRequest('http://localhost/api/tasks'));
    ...
    it('UC-TASK-03: POST creates a task (write auth required)', async () => {
        ...
        vi.mocked(requireWriteAuth).mockResolvedValue({ ok: true, session: session() });
        vi.mocked(requireWrite).mockResolvedValue({ ok: true, session: session() });
        const res = await POST(authedRequest('http://localhost/api/tasks', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    it('UC-TASK-04: PATCH updates status for an owned task', async () => {
        ...
        vi.mocked(requireWriteAuth).mockResolvedValue({ ok: true, session: session() });
        vi.mocked(requireWrite).mockResolvedValue({ ok: true, session: session() });
        const res = await PATCH(authedRequest('http://localhost/api/tasks', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
