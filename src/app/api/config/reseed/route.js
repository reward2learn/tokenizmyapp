...
import { CONFIG_UPLOAD_FIELD_NAMES, MAX_EXCEL_BYTES, MAX_MARKDOWN_BYTES, validateExcelUpload, validateMarkdownUpload, } from '@/lib/config/upload-validation';
import { seedFromSources } from '@/domain/seed/seed-runner';
...
function filesFromForm(formData, key) {
    const files = [];
...
export async function POST(request) {
    ...
    const executiveSummaryFile = fileFromForm(formData, CONFIG_UPLOAD_FIELD_NAMES.executiveSummary);
    ...
    try {
        ...
        if (excelFiles.length > 0) {
            ...
            overrides.excel = await Promise.all(excelFiles.map((f) => f.arrayBuffer().then((buf) => Buffer.from(buf))));
            uploaded.push('excel');
        }
