...
export async function POST(request) {
    ...
    try {
        ...
        if (!cached) {
            return jsonError('No cached workbook found. Upload the workbook via the Source tab first (Upload & reseed).', 404);
        }
        const excelBuffers = [Buffer.from(cached.content, 'base64')];
