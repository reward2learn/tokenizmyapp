...
export async function GET(request) {
    ...
    try {
        const rows = await db.$queryRawUnsafe(`SELECT id, sub, email, name, tier, role_code, is_active, last_seen_at, created_at
       FROM user_accounts
...
async function resolveCapabilities(db, sub) {
    try {
...
export async function POST(request) {
    ...
    const { id, email, isActive, roleCode, groupCodes, pin } = (body ?? {});
    ...
    try {
        if (typeof isActive === 'boolean' || roleCode !== undefined || email !== undefined) {
            await db.$executeRawUnsafe(`UPDATE user_accounts
         SET is_active = COALESCE($1, is_active),
             role_code = COALESCE($2, role_code),
             email = COALESCE($3, email),
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $4;`, typeof isActive === 'boolean' ? isActive : null, roleCode === null ? null : (roleCode ?? null), email === undefined ? null : email, id);
        }
        if (Array.isArray(groupCodes)) {
            await db.$executeRawUnsafe(`DELETE FROM user_groups WHERE user_id = $1;`, id);
            for (const code of groupCodes) {
                await db.$executeRawUnsafe(`INSERT INTO user_groups (id, user_id, group_id)
           SELECT gen_random_uuid()::TEXT, $1, sg.id FROM security_groups sg WHERE sg.code = $2
           ON CONFLICT (user_id, group_id) DO NOTHING;`, id, code);
            }
        }
        // Set PIN for this user (stored as USER_PIN_<sub> in secrets table).
        if (pin && pin.trim().length >= 3) {
            const user = await db.$queryRawUnsafe(`SELECT sub FROM user_accounts WHERE id = $1;`, id);
