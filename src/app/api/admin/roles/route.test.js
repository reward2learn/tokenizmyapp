...
function makeDb() {
    ...
    return {
        role: {
            ...
            findUnique: vi.fn(async ({ where }) => roles.find((r) => r.code.toLowerCase() === String(where.code).toLowerCase()) ?? null),
        },
    };
}
...
describe('/api/admin/roles', () => {
    beforeEach(() => {
        ...
        vi.clearAllMocks();
        vi.mocked(createClient).mockReturnValue(makeDb());
    });
    ...
    it('UC-ADMIN-01: GET lists roles with pinConfigured flag', async () => {
        const { requireWriteAuth } = await import('@/lib/auth/guards');
        vi.mocked(requireWriteAuth).mockResolvedValue({ ok: true, session: adminSession() });
        vi.mocked(getSecretPlaintext).mockImplementation(async (key) => key === 'USER_PIN_ama' ? '4271' : null);
        const res = await GET(new Request('http://localhost/api/admin/roles'));
    ...
    it('UC-ADMIN-02: POST sets a PIN for a known role', async () => {
        ...
        const res = await POST(new Request('http://localhost/api/admin/roles', {
            method: 'POST',
