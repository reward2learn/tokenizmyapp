...
export async function POST(request) {
    ...
    try {
        await db.$executeRawUnsafe(`INSERT INTO security_groups (code, name, description, is_system, permissions)
       VALUES ($1, $2, $3, false, $4)
       ON CONFLICT (code) DO UPDATE SET name = $2, description = $3, permissions = $4;`, code, name, description ?? null, perms);
...
export async function PATCH(request) {
    ...
    try {
        const affected = await db.$executeRawUnsafe(`UPDATE security_groups
       SET name = COALESCE($2, name),
           description = COALESCE($3, description),
           permissions = $4
       WHERE code = $1;`, code, name ?? null, description ?? null, perms);
        if (affected === 0)
...
function normalizePermissions(permissions) {
    ...
    for (const p of permissions) {
        if (typeof p === 'string' && VALID_CAPABILITIES.includes(p))
            set.add(p);
    }
