...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    await db.$executeRawUnsafe(
      // `id` is @default(cuid()) — generated by the Prisma client, so the column
      // has no database default and a raw INSERT must supply one itself.
      `INSERT INTO security_groups (id, code, name, description, is_system, permissions, tenant_slug, app_id)
