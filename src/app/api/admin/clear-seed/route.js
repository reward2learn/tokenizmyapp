...
export async function POST(request) {
    ...
    if (!parsed.success) {
        return jsonError('Send { "mode": "all", "confirm": "CLEAR ALL SEEDED DATA" } or ' +
            '{ "mode": "selected", "tables": [...], "confirm": "CLEAR SELECTED" }', 400);
    }
    const tablesToDelete = parsed.data.mode === 'all'
        ? tableNames
        : parsed.data.tables;
