...
async function resolveWorkbook() {
    ...
    throw new Error('Workbook file not found on disk and no cached copy in database. ' +
...
export async function GET(request) {
    try {
        ...
        const promptPreview = fullPrompt.length > 3000
...
export async function POST(request) {
    ...
    if (!parsed.success) {
        return NextResponse.json({ success: false, error: 'Invalid request body', details: parsed.error.flatten() }, { status: 400 });
    }
    ...
    const dbSession = { tier: guard.session.tier, sub: guard.session.sub };
    ...
    if (filePath) {
        ...
        try {
            ...
            try {
                ...
                if (cached?.content) {
                    const buffers = [Buffer.from(cached.content, 'base64')];
    ...
    try {
        ...
        if (!result.success) {
            return NextResponse.json({ success: false, error: result.error, prompt: result.prompt }, { status: 500 });
        }
        return NextResponse.json({
            success: true,
            data: {
