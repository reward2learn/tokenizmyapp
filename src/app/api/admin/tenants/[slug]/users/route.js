...
import { ensureTenantUserColumn } from '@/domain/tenant/tenant-user-service';
import { resolveCapabilitiesForSub } from '@/domain/security/security-service';
import { setSecret } from '@/lib/secrets';
export const dynamic = 'force-dynamic';
export const maxDuration = 30;
// ── Helpers ────────────────────────────────────────────
async function resolveGroups(db, userId) {
    try {
        const rows = await db.$queryRawUnsafe(`SELECT sg.code FROM security_groups sg
...
async function resolveCaps(db, sub) {
    try {
...
function toView(r, groups, permissions) {
    return {
        id: r.id,
        sub: r.sub,
        email: r.email,
...
export async function GET(request, { params }) {
    ...
    try {
        ...
        const tenantRows = await db.$queryRawUnsafe(`SELECT id FROM tenants WHERE slug = $1 LIMIT 1;`, slug);
        if (tenantRows.length === 0)
            return jsonError('Tenant not found', 404);
        const rows = await db.$queryRawUnsafe(`SELECT id, sub, email, name, tier, role_code, is_active, last_seen_at, created_at, tenant_slug
       FROM user_accounts
       WHERE tenant_slug = $1
...
export async function POST(request, { params }) {
    ...
    try {
        ...
        // Upsert user
        const existing = await db.$queryRawUnsafe(`SELECT id FROM user_accounts WHERE sub = $1 AND tenant_slug = $2 LIMIT 1;`, sub, slug);
        let userId;
        if (existing.length > 0) {
            userId = existing[0].id;
            await db.$executeRawUnsafe(`UPDATE user_accounts SET
         ...
         WHERE id = $6;`, email ?? null, name ?? null, tier ?? null, roleCode === null ? null : (roleCode ?? null), isActive === null ? null : isActive, userId);
        }
        else {
            userId = `usr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
            await db.$executeRawUnsafe(`INSERT INTO user_accounts (id, sub, email, name, tier, role_code, is_active, tenant_slug, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);`, userId, sub, email ?? null, name ?? null, tier ?? 'pin', roleCode ?? null, isActive ?? true, slug);
        }
        // Handle group membership
        if (Array.isArray(groupCodes)) {
            await db.$executeRawUnsafe(`DELETE FROM user_groups WHERE user_id = $1;`, userId);
            for (const code of groupCodes) {
                await db.$executeRawUnsafe(`INSERT INTO user_groups (user_id, group_id)
           SELECT $1, sg.id FROM security_groups sg WHERE sg.code = $2
           ON CONFLICT (user_id, group_id) DO NOTHING;`, userId, code);
            }
        }
        // Set PIN if provided
...
export async function DELETE(request, { params }) {
    ...
    try {
        ...
        const result = await db.$executeRawUnsafe(`DELETE FROM user_accounts WHERE id = $1 AND tenant_slug = $2;`, id, slug);
        if (Number(result) === 0) {
