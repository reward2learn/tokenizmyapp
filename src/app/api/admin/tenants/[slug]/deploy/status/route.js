...
export async function GET(request, { params }) {
    ...
    try {
        ...
        const projectRes = await fetch(`${VERCEL_API}/v10/projects/${slug}?teamId=${TEAM_ID}`, { headers: { Authorization: `Bearer ${token}` } });
        if (!projectRes.ok) {
            return jsonOk({
                state: 'NOT_FOUND',
                slug,
        ...
        const deployRes = await fetch(`${VERCEL_API}/v6/deployments?projectId=${project.id}&target=production&limit=1&teamId=${TEAM_ID}`, { headers: { Authorization: `Bearer ${token}` } });
        if (!deployRes.ok) {
            return jsonOk({
        ...
        if (latest.state === 'ERROR' || latest.state === 'CANCELED') {
            try {
                const evRes = await fetch(`${VERCEL_API}/v1/deployments/${latest.uid}/events?teamId=${TEAM_ID}&limit=5`, { headers: { Authorization: `Bearer ${token}` } });
                if (evRes.ok) {
