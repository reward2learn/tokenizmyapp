...
import { getDepartment, legacyAliases, requiredForDepartment, sectionsForDepartment, Z_REPORT_DEPARTMENTS, Z_REPORT_RECEIPT_SECTIONS, Z_REPORT_SECTIONS, } from '@/domain/z-report/z-report-schema';
...
function normalizeRow(row) {
    ...
    if (!r.department)
        r.department = 'all_pos';
    if (r.receipt_images && typeof r.receipt_images === 'string') {
        try {
            r.receipt_images = JSON.parse(r.receipt_images);
        }
        catch {
            r.receipt_images = [];
        }
    }
    if (!Array.isArray(r.receipt_images))
        r.receipt_images = [];
    return r;
}
function rowToApi(row) {
    ...
    return legacyAliases(stripped ?? normalized);
}
export async function GET(request) {
    ...
    if (url.searchParams.has('detail')) {
        ...
        try {
            ...
            if (!row)
                return legacyError('Entry not found', 404);
            const normalized = normalizeRow(row);
            return NextResponse.json({
                success: true,
    ...
    const limit = Math.min(exportAll ? 5000 : 100, Math.max(1, parseInt(url.searchParams.get('limit') ?? '10', 10) || 10));
    const page = Math.max(1, parseInt(url.searchParams.get('page') ?? '1', 10) || 1);
    const offset = (page - 1) * limit;
    const where = {};
    if (from)
        where.reportDate = { ...where.reportDate, gte: new Date(`${from}T00:00:00.000Z`) };
    if (to)
        where.reportDate = { ...where.reportDate, lte: new Date(`${to}T00:00:00.000Z`) };
...
export async function POST(request) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, data: rowToApi(saved) }, { status: 201 });
    }
    catch (err) {
        ...
        return NextResponse.json({ success: false, error: 'Insert failed', detail: err instanceof Error ? err.message : String(err) }, { status: 500 });
    }
}
export async function DELETE(request) {
    ...
    try {
        ...
        return NextResponse.json({ success: false, error: 'Delete failed', detail: err instanceof Error ? err.message : String(err) }, { status: 500 });
    }
}
