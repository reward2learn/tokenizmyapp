...
describe('GET /api/vjobs/status/[jobId]', () => {
    it('returns job status shape for pending jobs', async () => {
        const response = await GET(new Request('http://localhost/api/vjobs/status/job-abc'), { params: Promise.resolve({ jobId: 'job-abc' }) });
    ...
    it('returns 404 when job not found', async () => {
        ...
        const response = await GET(new Request('http://localhost/api/vjobs/status/missing'), { params: Promise.resolve({ jobId: 'missing' }) });
