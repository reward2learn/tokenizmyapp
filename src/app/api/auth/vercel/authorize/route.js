...
export async function GET(request) {
    if (!CLIENT_ID) {
        return NextResponse.json({ error: 'Vercel OAuth is not configured — missing NEXT_PUBLIC_VERCEL_APP_CLIENT_ID' }, { status: 500 });
    }
    ...
    const response = NextResponse.redirect(`https://vercel.com/oauth/authorize?${new URLSearchParams({
        client_id: CLIENT_ID,
