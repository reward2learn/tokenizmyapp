...
import { buildGoogleAuthUrl, getGoogleOAuthCredentials, getGoogleOAuthPublicConfig, } from '@/lib/auth/google-oauth';
...
import { clearSessionCookie, getOrigin, getSessionFromRequest, setSessionCookie, } from '@/lib/auth/session';
...
let jobQueueEnsured = null;
function ensureJobQueueOnce() {
    if (!jobQueueEnsured) {
...
async function handleGoogleConfig() {
...
async function handleGoogleRedirect(request, url) {
...
async function handleGoogleCallback(request, url) {
    ...
    try {
        ...
        const platformAdmin = (matchedRole?.isPlatformAdmin ?? false) || groups.includes('platform-admin');
...
async function handleVerifyPin(request) {
    let body;
    try {
 ...
 */
async function handleListPinUsers() {
    // Best-effort: read last_seen_at from Neon for PIN preference.
    let lastSeenBySub = new Map();
    try {
        if (process.env.POSTGRES_URL || process.env.DATABASE_URL) {
            const db = createBaseClient();
            await ensureSecurityTables(db);
            const rows = await db.$queryRawUnsafe(`SELECT sub, last_seen_at FROM user_accounts WHERE last_seen_at IS NOT NULL;`);
            for (const row of rows ?? []) {
                if (row.last_seen_at)
                    lastSeenBySub.set(row.sub, new Date(row.last_seen_at));
            }
        }
    }
    catch (err) {
...
async function handlePdf(request, url) {
