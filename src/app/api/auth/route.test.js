...
describe('/api/auth', () => {
    ...
    it('UC-AUTH-02: verify-pin sets session on correct PIN', async () => {
        ...
        const response = await POST(new Request('http://localhost/api/auth?action=verify-pin', {
            method: 'POST',
    ...
    it('UC-AUTH-08: store-key requires SETUP_TOKEN bearer', async () => {
        ...
        const response = await POST(new Request('http://localhost/api/auth?action=store-key', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
