...
import { FinancialProjectionService, SCENARIO_MAP, } from '@/domain/financial/financial-projection-service';
import { MonthlyActualsService } from '@/domain/actuals/monthly-actuals-service';
import { ACTUALS_COST_DEPARTMENTS, getActualsDepartment, sectionForDepartment, } from '@/domain/actuals/monthly-actuals-schema';
import { buildComputedPnl, computedPreview, COMPUTED_PREVIEW_KEYS, isExcelLedgerMonth, MANUAL_INPUT_SECTIONS, } from '@/domain/financial/pnl-calculator';
...
async function buildMonthlyActualsPayload(service, sync, prefillHelper, period, departmentId) {
...
async function handleMonthlyActuals(request, url) {
    ...
    if (request.method === 'POST') {
        ...
        try {
            if (department === 'all') {
                ...
                const payload = await buildMonthlyActualsPayload(writeService, new SyncMonthlyActuals(writeDb), new MonthlyActualsPrefill(writeDb), period, 'all');
                return NextResponse.json({
                    success: true,
            ...
            if (department) {
                ...
                const payload = await buildMonthlyActualsPayload(writeService, new SyncMonthlyActuals(writeDb), new MonthlyActualsPrefill(writeDb), period, department);
                return NextResponse.json({
                    success: true,
            ...
            const payload = await buildMonthlyActualsPayload(writeService, new SyncMonthlyActuals(writeDb), new MonthlyActualsPrefill(writeDb), period, null);
...
async function handleReports(url) {
    ...
    try {
        ...
        ORDER BY period_start ASC`;
            ...
            return NextResponse.json({
                ...
                data: {
                    period,
                    metrics: rows.map((row) => serializeReportRow(row)),
                    targets,
                },
            });
        }
...
export async function GET(request) {
    ...
    try {
        const scenario = (url.searchParams.get('scenario') || 'conservative');
        const data = await service.getChartOverview(SCENARIO_MAP[scenario] ? scenario : 'conservative');
        return NextResponse.json({ success: true, data });
    }
    catch (err) {
