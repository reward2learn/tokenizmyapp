/**
 * Z Sales Day Report — field definitions matching SPICERY POS printout.
 * Used by api/metrics.js, pos-extract.js, and ops-admin.html (via script tag).
 */

export const Z_REPORT_SECTIONS = [
  {
    id: 'header',
    title: 'Report Header',
    fields: [
...
/** Printed on the POS Z Sales Day Report receipt (has sample `pos` value). */
export function isPosReceiptField(field) {
  return field != null && field.pos != null && String(field.pos).trim() !== '';
}

export function filterReceiptSections(sections) {
  return (sections || Z_REPORT_SECTIONS)
    .map((section) => ({
      ...section,
...
export const Z_REPORT_RECEIPT_FIELD_KEYS = Z_REPORT_RECEIPT_SECTIONS.flatMap((s) =>
...
/** Daily reporting departments — each has its own receipt upload and field subset. */
export const Z_REPORT_DEPARTMENTS = [
  {
    id: 'all_pos',
...
export function sectionsForDepartment(deptId, receiptOnly = false) {
...
export function requiredForDepartment(deptId) {
  return getDepartment(deptId).required || Z_REPORT_REQUIRED;
}

/** SQL column types for migration */
export const Z_REPORT_SQL_TYPES = {
...
const INT_KEYS = new Set(Z_REPORT_FIELD_KEYS.filter((k) => k.endsWith('_qty') || k === 'report_no' || k === 'total_bills' || k === 'total_covers'));
const AMOUNT_KEYS = new Set(Z_REPORT_FIELD_KEYS.filter((k) => k.endsWith('_amount') || k === 'total_sales' || k === 'estimated_sales' || k === 'nett_sales' || k === 'avg_bills' || k === 'avg_covers'));
const TEXT_KEYS = new Set(['operator', 'pos_group', 'begin_receipt_no', 'end_receipt_no', 'raw_text']);

export function sqlTypeForKey(key) {
  if (key === 'report_date') return 'DATE NOT NULL UNIQUE';
  if (key === 'report_time') return 'TIME';
  if (key === 'period_start' || key === 'period_end') return 'TIMESTAMP';
  if (INT_KEYS.has(key)) return 'INTEGER DEFAULT 0';
  if (AMOUNT_KEYS.has(key)) return 'NUMERIC(14,2) DEFAULT 0';
...
export function buildCreateTableSql() {
  ...
  for (const key of Z_REPORT_FIELD_KEYS) {
    if (key === 'report_date') {
      cols.push('report_date DATE NOT NULL UNIQUE');
      continue;
    }
    cols.push(`${key} ${sqlTypeForKey(key)}`);
  }
  cols.push('raw_text TEXT');
  cols.push('created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP');
  return `CREATE TABLE IF NOT EXISTS daily_z_reports (\n  ${cols.join(',\n  ')}\n);`;
}

export function legacyAliases(row) {
  if (!row) return row;
  return {
    ...row,
