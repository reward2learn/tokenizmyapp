...
# Opt-out only: SKIP_BUILD_DB_SYNC=1 (e.g. temporary Neon quota issues).
# Default: sync schema whenever POSTGRES_URL is set (local + Vercel).
if [ -n "$POSTGRES_URL" ]; then
  case "${SKIP_BUILD_DB_SYNC:-}" in
    1|true|TRUE|yes|YES)
      echo "[vercel-build] SKIP_BUILD_DB_SYNC set — skipping schema sync."
      ;;
    *)
      # Prefer direct Neon URL for build-time schema sync (pooler refuses
      # connections while compute is waking). Fall back to pooled POSTGRES_URL.
