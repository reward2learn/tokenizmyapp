...
function buildMonthlyVarianceSection(data: ExcelData): string {
  ...
  return [
    `## Monthly Variance`,
    ``,
    `| Item | Prior Period | Current Period | Variance | Variance % |
|------|--------------|----------------|----------|------------|
${tableRows.join('\n')}`,
    ``,
  ].join('\n');
...
export function buildGenerationPrompt(data: ExcelData, additionalContext?: string): string {
  ...
  sections.push(`Return ONLY a JSON object with "businessReview" and "executiveSummary" (Markdown strings).`);
  sections.push(`Also include "homeHero" and "tasks" when producing the full content package.`);
  sections.push(`Base ALL numbers on the data provided above. Do not fabricate data.`);
  sections.push(``);
